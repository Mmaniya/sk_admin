...
$("form").submit(function(event){
	...
	if(required_fields_filled){
		...
		alert(data);
			dataArr = data.split('::');
			if(dataArr[0]=="quests"){
				$('#'+dataArr[1]).removeAttr('disabled').trigger('click');
			}
			if(dataArr[0]=="ok"){
				window.location.href =dataArr[1]+'.php';
			}else if(dataArr[0]=="redirect"){
				window.location.href =dataArr[1]+'.php';				
			}else if(dataArr[0]=="le"){
...
$(document).ready(function () {
  ...
  allNextBtn.click(function(){
      ...
      var curStep = $(this).closest(".setup-content"),
          ...
          curInputs = curStep.find("input[type='text'],select,textarea"),
          isValid = true;
      ...
      for(var i=0; i<curInputs.length; i++){
          if (!curInputs[i].validity.valid){
			  ...
			  $('html, body').animate({scrollTop: ($(validator.errorList[0].element).offset()).top}, 5000);
          }
      }
