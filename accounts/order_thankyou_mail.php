   ...
   <tr>
	...
	/*
		...
		*/
		...
		$serviceObj->invoice_id = $rsInvPayment->invoice_id;
		$servicesOrdered = $serviceObj->getServicesByInvoiceId();
		$invServices='';
			foreach($servicesOrdered as $K1=>$V1) { 	
				if(is_array($V1)) { 
				   $invServices.=$V1['item_name'].', ';
				}
			}
			
		 
		 $loginUrl = CLIENT_URL;
		$search = array('[ORDER_AMOUNT]','[INVOICE_ID]','[INVOICE_DATE_TIME]','[TXN_ID]','[CLIENT_EMAIL]','[CLIENT_PASS]','[CLIENT_NAME]','[LOGIN_URL]');
		$replace = array(money($rsInvPayment->amount_paid,'$'),$rsInvPayment->invoice_id,date('M d,Y h:i A',strtotime($rsInvPayment->added_date)),$transactionID,$rsClient->client_email,
		                 $rsClient->client_pass,$rsClient->client_fname.' '.$rsClient->client_lname,ACCOUNT_LINK);		
	...
	//	use PHPMailer\PHPMailer\Exception;
		...
		require_once 'PHPMailer/src/SMTP.php';
		 ...
		 $emailSubject = "[BizPlanEasy] Order Id: ".$rsInvPayment->invoice_id." Thank you for your order, ".$rsClient->client_fname.' '.$rsClient->client_lname; 
		...
		try {
			...
			$mail->AddAddress('support@bizplaneasy.com');
			$mail->AddBCC('kavitharjn@gmail.com');
			//$emailSubject = "[BizPlanEasy] Order Id: ".$rsInvPayment->invoice_id." Payment Successful "; 
			 $emailSubject = "[BizPlanEasy] YOU’VE GOT MONEY : ".$rsClient->client_fname.' '.$rsClient->client_lname." $".$rsInvPayment->amount_paid."-".$invServices; 
			$mail->Subject =  $emailSubject;                
		...
		$mail->AddAttachment($_SERVER['DOCUMENT_ROOT']."invoice_pdf/".$fileName.".pdf");
		$mail->send();		
