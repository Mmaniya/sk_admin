...
<div class="card" style="margin-top:20px">    
  <div class="card-body">
  <h5><?php echo  $lineLineItemName; ?></h5>
...
<p class="card-header bg-primary text-white">Upload Files  <?php echo $lineLineItemName; ?></p> <br/>
    ...
    <form name="upload_document_form" id="upload_document_form"   enctype="multipart/form-data">
		...
		<?php } else { ?> <input type="hidden" name="line_item_id" id="line_item_id" value="<?php echo $invoice_line_item_id; ?>"> 
		...
		<?php } ?>
	     ...
	     <div class="row">
			...
			<span class="document_appended_file"></span>
			 ...
			 <button type="submit" class="btn btn-danger btn-sm"  style="padding:5px;font-size: 16px;">Upload files</button>
			 </div>
   </form>
</div>
</div>
...
<script>
 ...
 $( "form#upload_document_form" ).submit(function( event ) {
  ...
  list_client_documents($('#line_item_id').val());  $('#con-close-modal').modal('hide');  $('.modal-backdrop').css('display','none');
