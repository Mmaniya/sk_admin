...
if($page > 1)
	...
	if(count($rsDiscount)>0 && $totalPages > 1){
	if($page > 1){
		$pageNo = $page - 1;
...
<div class="col-md-2 d-none d-sm-block">Discount Name</div>
...
<?php   $i = 0;
 ...
 if($val->discount_type=='P') { $discountType = 'Percentage';  }    
 if($val->discount_type=='F') { $discountType = 'Flat';  }    
 $bg='';  $i++;  
...
<div class="row right_border tb_row" style="background-color:<?php echo $bg; ?> ">
 <div class="col-md-1 d-none d-sm-block"><?php echo $key+1;?></div>
  <div class="col-md-2 colBorder"><strong><?php echo $val->discount_name.' <br/>('.$val->discount_code.')';?></strong></div>
  <div class="col-md-6 colBorder"><strong><?php echo $discountType.' : '.$val->discount_value; ?> </strong><br/>
   From : <strong><?php echo date('M d, Y',strtotime($val->valid_from));  ?></strong>  To :  <strong><?php echo  date('M d, Y',strtotime($val->valid_to));  ?></strong>
   <br/>
   <?php  echo $val->discount_desc;  ?>
  </div>
  <div class="col-md-3 colBorder"><a href="javascript:void(0)" class="a_tag" onclick="showAddEditDiscount(<?php echo $val->id; ?>)">[edit]</a> 
				<a href="javascript:void(0)" class="a_tag" onclick="deleteDiscount(<?php echo $val->id; ?>)">[delete]</a>  
				<?php if($val->status=='I') { ?>
...
<script> 
  ...
  function ShowListPagination(page) { 
	 paramData = {'act':'show_discount_list','page':page}
	ajax({ 
		a:'discount',
		b:$.param(paramData),
		c:function(){},
...
function setActiveCategory(id) {
	...
	paramData = {'act':'setActiveDiscount','id':id,'status':setstatus};
	ajax({ 
		a:'discount',
		b:$.param(paramData),
		c:function(){},
